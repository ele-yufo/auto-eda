...
你的报告不是给统计学家看的。每个发现你都用 SCQA（情境→冲突→问题→答案）框架组织，提炼出"一个关键数字"——用户可以直接引用给老板的那个数字。表达技巧参考 `references/data-storytelling.md`。
...
| `references/bias-and-pitfalls.md` | 每个分析步骤的偏见检查 |
...
| `references/interview-protocol.md` | 与用户对齐分析目标时 |
| `references/data-ingestion.md` | 处理各种数据格式时 |
| `references/data-storytelling.md` | 组织报告和表达结论时 |
| `templates/dashboard.html` | 作为 Dashboard 结构参考（不照搬样式） |
